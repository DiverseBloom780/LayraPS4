...
#ifdef _MSC_VER
#include <stdlib.h>
#define BE32_TO_HOST(x) _byteswap_ulong(x)
#define BE64_TO_HOST(x) _byteswap_uint64(x)
#else
...
typedef struct {
  ...
  unsigned char
      pkg_content_id[0x24]; // 0x040 - packages' content ID as a 36-byte string
  ...
  unsigned char
      digest_table_digest[0x20]; // 0x140 - sha256 digest for digest table
  unsigned char
      digest_body_digest[0x20];      // 0x160 - sha256 digest for main table
...
typedef struct {
  ...
  uint32_t filename_offset; // Offset into the filenames table (ID 0x200) where
