...
#ifndef MAX_PATH
#ifdef _WIN32
// This is the maximum number of UTF-16 code units permissible in Windows file
// paths
#define MAX_PATH 260
#include <Shlobj.h>
#include <windows.h>
#else
// This is the maximum number of UTF-8 code units permissible in all other OSes'
// file paths
...
namespace Common_Filesystem = std::filesystem;
namespace fs = std::filesystem;

#ifdef __APPLE__
using IsTranslocatedURLFunc = Boolean (*)(CFURLRef path, bool *isTranslocated,
                                          CFErrorRef *__nullable error);
using CreateOriginalPathForURLFunc = CFURLRef
  ...
  if (CFBundleRef bundle_ref = CFBundleGetMainBundle()) {
    if (CFURLRef bundle_url_ref = CFBundleCopyBundleURL(bundle_ref)) {
      ...
      if (CFURLGetFileSystemRepresentation(
              untranslocated_url_ref ? untranslocated_url_ref : bundle_url_ref,
              true, reinterpret_cast<u8 *>(app_bundle_path),
...
static auto UserPaths = [] {
  ...
  if (!std::filesystem::exists(user_dir)) {
...
#ifdef __APPLE__
    user_dir = std::filesystem::path(getenv("HOME")) / "Library" /
    ...
    if (xdg_data_home != nullptr && strlen(xdg_data_home) > 0) {
      ...
      user_dir = std::filesystem::path(getenv("HOME")) / ".local" / "share" /
                 "shadPS4";
    }
#elif _WIN32
  ...
  if (notice_file.is_open()) {
    notice_file
        << "++++++++++++++++++++++++++++++++\n+ Custom Trophy Images / Sound "
           ...
           "to the "
           "trophies.\n*We recommend a square resolution image, for example "
           "200x200, 500x500, "
           ...
           "names:\n\nbronze.png\nsilver.png\ngold.png\nplatinum.png\n\nYou "
           "can add a custom "
           "sound for trophy notifications.\n*By default, no audio is played "
           "unless it is in "
           "this folder and you are using the QT version.\nIn this folder "
           "('user\\custom_trophy'), add the files with the following "
           "names:\n\ntrophy.mp3";
    notice_file.close();
  }
...
bool ValidatePath(const fs::path &path) {
  if (path.empty()) {
    LOG_ERROR("Filesystem", "Input path is empty, path={}",
  ...
  if (path.u16string().size() >= MAX_PATH) {
    LOG_ERROR("Filesystem", "Input path is too long, path={}",
              PathToUTF8String(path));
    return false;
  }
#else
...
FindGameByID(const fs::path &dir, const std::string &game_id, int max_depth) {
  if (max_depth < 0) {
    return std::nullopt;
  }
