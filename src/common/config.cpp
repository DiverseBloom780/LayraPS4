...
Core::Networking::Config getNetworkingConfig() {
    return networkingConfig.get();
}

void setNetworkingConfig(const Core::Networking::Config& config, bool is_game_specific) {
    networkingConfig.set(config, is_game_specific);
}
...
void load(const std::filesystem::path& path, bool is_game_specific) {
  ...
  // General
    volumeSlider.setFromToml(tbl, "VolumeSlider", is_game_specific);

    // Networking
    if (auto net_tbl = toml::get_optional<toml::table>(tbl, "Networking")) {
        Core::Networking::Config current_config = networkingConfig.get();
        if (auto mode = toml::get_optional<int>(*net_tbl, "Mode")) {
            current_config.mode = (Core::Networking::Mode)*mode;
        }
        if (auto relay = toml::get_optional<std::string>(*net_tbl, "LANPlayRelayAddress")) {
            current_config.lan_play_relay_address = *relay;
        }
        if (auto ip = toml::get_optional<std::string>(*net_tbl, "GuestIP")) {
            current_config.guest_ip = *ip;
        }
        if (auto ezfn = toml::get_optional<std::string>(*net_tbl, "EZFNServerAddress")) {
            current_config.ezfn_server_address = *ezfn;
        }
        if (auto gtav = toml::get_optional<std::string>(*net_tbl, "GTAVServerAddress")) {
            current_config.gtav_server_address = *gtav;
        }
        if (auto spoof = toml::get_optional<bool>(*net_tbl, "SpoofPSN")) {
            current_config.spoof_psn = *spoof;
        }
        networkingConfig.set(current_config, is_game_specific);
...
void sortTomlSectitoml::ordered_value data = toml::ordered_value::table();

    // Networking
    data["Networking"]["Mode"] = (int)networkingConfig.get().mode;
    data["Networking"]["LANPlayRelayAddress"] = networkingConfig.get().lan_play_relay_address;
    data["Networking"]["GuestIP"] = networkingConfig.get().guest_ip;
    data["Networking"]["EZFNServerAddress"] = networkingConfig.get().ezfn_server_address;
    data["Networking"]["GTAVServerAddress"] = networkingConfig.get().gtav_server_address;
    data["Networking"]["SpoofPSN"] = networkingConfig.get().spoof_psn; ordered_data;
