...
private:
  ...
  static constexpr float BOOT_DURATION = 3.0f; // 3 second boot sequence
...
public:
  ...
  void Render() {
    ...
    ImVec2 logo_pos((display_size.x - logo_size.x) / 2,
                    (display_size.y - logo_size.y) / 2);

    // Draw placeholder logo (you'll replace this with actual texture loading)
    ImDrawList *draw_list = ImGui::GetWindowDrawList();
    draw_list->AddRectFilled(
        logo_pos, ImVec2(logo_pos.x + logo_size.x, logo_pos.y + logo_size.y),
        IM_COL32(0, 100, 255, 255), 10.0f);

    // Add "LayraPS4" text below logo
    ImGui::SetCursorPos(ImVec2(logo_pos.x + (logo_size.x - 200) / 2,
                               logo_pos.y + logo_size.y + 20));
  ...
  bool IsBootComplete() const { return !show_boot_screen; }
...
void boot_screen_init(void) { g_boot_screen = BootScreen(); }

void boot_screen_render(void) { g_boot_screen.Render(); }

bool boot_screen_is_complete(void) { return g_boot_screen.IsBootComplete(); }

void boot_screen_reset(void) { g_boot_screen.Reset(); }
