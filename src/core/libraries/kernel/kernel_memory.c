...
extern PAddr MemoryManager_Allocate(void* memory, s64 searchStart, s64 searchEnd, u64 len,
                                   u64 alignment, s32 memoryType);
extern s32 MemoryManager_Free(void* memory, u64 start, u64 len, bool checked);
extern s32 MemoryManager_DirectQueryAvailable(void* memory, u64 searchStart, u64 searchEnd,
                                             u64 alignment, PAddr* physAddr, u64* size);
extern s32 MemoryManager_VirtualQuery(void* memory, VAddr addr, s32 flags, OrbisVirtualQueryInfo* info);
extern s32 MemoryManager_MapMemory(void* memory, void** addr, VAddr in_addr, u64 len,
                                  s32 prot, s32 flags, s32 vma_type, const char* name,
                                  bool should_check, s64 phys_addr, u64 alignment);
extern s32 MemoryManager_QueryProtection(void* memory, VAddr addr, void** start, void** end, u32* prot);
extern s32 MemoryManager_Protect(void* memory, VAddr addr, u64 size, s32 prot);
extern s32 MemoryManager_DirectMemoryQuery(void* memory, u64 offset, bool extended, OrbisQueryInfo* info);
extern u64 MemoryManager_GetAvailableFlexibleSize(void* memory);
extern s32 MemoryManager_GetDirectMemoryType(void* memory, u64 addr, s32* typeOut,
                                            void** startOut, void** endOut);
extern s32 MemoryManager_IsStack(void* memory, VAddr addr, void** start, void** end);
extern void MemoryManager_SetDirectMemoryType(void* memory, VAddr addr, u64 len, s32 type);
extern void MemoryManager_NameVirtualRange(void* memory, VAddr addr, u64 len, const char* name);
...
extern PAddr MemoryManager_PoolExpand(void* memory, u64 searchStart, u64 searchEnd, u64 len, u64 alignment);
extern s32 MemoryManager_PoolCommit(void* memory, VAddr addr, u64 len, s32 prot, s32 type);
extern s32 MemoryManager_PoolDecommit(void* memory, VAddr addr, u64 len);
extern void MemoryManager_GetMemoryPoolStats(void* memory, OrbisKernelMemoryPoolBlockStats* stats);
extern s32 MemoryManager_MapFile(void* memory, void** addr, VAddr in_addr, u64 len,
                                s32 prot, s32 flags, s32 fd, s64 offset);
extern void MemoryManager_SetPrtArea(void* memory, s32 id, VAddr address, u64 size);
...
s32 sceKernelAllocateDirectMemory(s64 searchStart, s64 searchEnd, u64 len,
    ...
    if (physAddrOut == NULL) {
        fprintf(stderr, "[Kernel_Memory] ERROR: Result physical address pointer is null!\n");
        return ORBIS_KERNEL_ERROR_EINVAL;
    }

    bool is_in_range = (searchEnd - searchStart) >= (s64)len;
    if (searchEnd <= searchStart || searchEnd < (s64)len || !is_in_range) {
        fprintf(stderr, "[Kernel_Memory] ERROR: Provided address range is too small! "
    ...
    void* memory = MemoryManager_GetInstance();
    PAddr phys_addr = MemoryManager_Allocate(memory, searchStart, searchEnd, len, alignment, memoryType);
    ...
    *physAddrOut = (s64)phys_addr;
    ...
    printf("[Kernel_Memory] sceKernelAllocateDirectMemory: searchStart = 0x%lx, searchEnd = 0x%lx, "
           "len = 0x%lx, alignment = 0x%lx, memoryType = 0x%x, physAddrOut = 0x%lx\n",
...
s32 sceKernelCheckedReleaseDirectMemory(u64 start, u64 len) {
    printf("[Kernel_Memory] sceKernelCheckedReleaseDirectMemory: start = 0x%lx, len = 0x%lx\n",
           start, len);
    if (!Is16KBAligned(start) || !Is16KBAligned(len)) {
        fprintf(stderr, "[Kernel_Memory] ERROR: Misaligned start or length, start = 0x%lx, length = 0x%lx\n",
                start, len);
        return ORBIS_KERNEL_ERROR_EINVAL;
    }
    if (len == 0) {
...
s32 sceKernelReleaseDirectMemory(u64 start, u64 len) {
    printf("[Kernel_Memory] sceKernelReleaseDirectMemory: start = 0x%lx, len = 0x%lx\n",
           start, len);
    if (!Is16KBAligned(start) || !Is16KBAligned(len)) {
        fprintf(stderr, "[Kernel_Memory] ERROR: Misaligned start or length, start = 0x%lx, length = 0x%lx\n",
                start, len);
...
s32 sceKernelAvailableDirectMemorySize(u64 searchStart, u64 searchEnd, u64 alignment,
    ...
    printf("[Kernel_Memory] sceKernelAvailableDirectMemorySize: searchStart = 0x%lx, "
    ...
    PAddr physAddr = 0;
    u64 size = 0;
    s32 result = MemoryManager_DirectQueryAvailable(memory, searchStart, searchEnd, alignment,
                                                    &physAddr, &size);
...
s32 sceKernelReserveVirtualRange(void** addr, u64 len, s32 flags, u64 alignment) {
    printf("[Kernel_Memory] sceKernelReserveVirtualRange: addr = %p, len = 0x%lx, "
    ...
    if (len == 0 || !Is16KBAligned(len)) {
        fprintf(stderr, "[Kernel_Memory] ERROR: Map size is either zero or not 16KB aligned!\n");
        return ORBIS_KERNEL_ERROR_EINVAL;
    }
    if (alignment != 0) {
    ...
    s32 result = MemoryManager_MapMemory(memory, addr, in_addr, len, 0, flags, 0, "anon",
                                        false, -1, alignment);
    if (result == 0) {
...
s32 sceKernelMapNamedDirectMemory(void** addr, u64 len, s32 prot, s32 flags,
    ...
    printf("[Kernel_Memory] sceKernelMapNamedDirectMemory: in_addr = %p, len = 0x%lx, "
           "prot = 0x%x, flags = 0x%x, phys_addr = 0x%lx, alignment = 0x%lx, name = '%s'\n",
    ...
    if (!Is16KBAligned(phys_addr)) {
        fprintf(stderr, "[Kernel_Memory] ERROR: Start address is not 16KB aligned!\n");
        return ORBIS_KERNEL_ERROR_EINVAL;
    }
    ...
    if (prot & 0x4) { // CPU_EXEC bit
        fprintf(stderr, "[Kernel_Memory] ERROR: Executable permissions are not allowed.\n");
    ...
    if (g_sdk_version >= 0x2500000 && !(flags & 0x200)) { // FW 2.5+, not Stack flag
        should_check = !g_alias_dmem;
    }
    
    // VMAType::Direct = 1
    s32 ret = MemoryManager_MapMemory(memory, addr, in_addr, len, prot, flags, 1, name,
...
s32 sceKernelMapDirectMemory(void** addr, u64 len, s32 prot, s32 flags, s64 phys_addr, u64 alignment) {
    printf("[Kernel_Memory] sceKernelMapDirectMemory (redirected to Named version)\n");
...
s32 sceKernelMapDirectMemory2(void** addr, u64 len, s32 type, s32 prot, s32 flags,
    ...
    printf("[Kernel_Memory] sceKernelMapDirectMemory2: in_addr = %p, len = 0x%lx, "
    ...
    // VMAType::Direct = 1
    s32 ret = MemoryManager_MapMemory(memory, addr, in_addr, len, prot, flags, 1, "anon",
...
s32 sceKernelMapNamedFlexibleMemory(void** addr_in_out, u64 len, s32 prot, s32 flags, const char* name) {
    printf("[Kernel_Memory] sceKernelMapNamedFlexibleMemory: in_addr = %p, len = 0x%lx, "
    ...
    // VMAType::Flexible = 2
    s32 ret = MemoryManager_MapMemory(memory, addr_in_out, in_addr, len, prot, flags, 2, name,
                                     false, -1, 0);
    printf("[Kernel_Memory] out_addr = %p\n", *addr_in_out);
    return ret;
}
...
s32 sceKernelMprotect(const void* addr, u64 size, s32 prot) {
    printf("[Kernel_Memory] sceKernelMprotect: addr = %p, size = 0x%lx, prot = 0x%x\n",
           addr, size, prot);
...
s32 sceKernelMtypeprotect(const void* addr, u64 size, s32 mtype, s32 prot) {
    printf("[Kernel_Memory] sceKernelMtypeprotect: addr = %p, size = 0x%lx, prot = 0x%x\n",
           addr, size, prot);
    ...
    if (result == ORBIS_OK) {
        MemoryManager_SetDirectMemoryType(memory, aligned_addr, aligned_size, mtype);
    }
    return result;
}

s32 sceKernelDirectMemoryQuery(u64 offset, s32 flags, OrbisQueryInfo* query_info, u64 infoSize) {
    printf("[Kernel_Memory] sceKernelDirectMemoryQuery: offset = 0x%lx, flags = 0x%x\n",
           offset, flags);
    void* memory = MemoryManager_GetInstance();
    return MemoryManager_DirectMemoryQuery(memory, offset, flags == 1, query_info);
}
...
s32 sceKernelGetDirectMemoryType(u64 addr, s32* directMemoryTypeOut,
    ...
    return MemoryManager_GetDirectMemoryType(memory, addr, directMemoryTypeOut,
                                            directMemoryStartOut, directMemoryEndOut);
}
...
s32 sceKernelBatchMap(OrbisKernelBatchMapEntry* entries, s32 numEntries, s32* numEntriesOut) {
    return sceKernelBatchMap2(entries, numEntries, numEntriesOut, ORBIS_KERNEL_MAP_FIXED);
}
...
s32 sceKernelBatchMap2(OrbisKernelBatchMapEntry* entries, s32 numEntries,
    ...
    for (s32 i = 0; i < numEntries; i++, processed++) {
        ...
        if (result != ORBIS_OK) {
            fprintf(stderr, "[Kernel_Memory] ERROR: Batch operation failed with error 0x%x\n", result);
            break;
        }
    }
...
s32 sceKernelMemoryPoolExpand(u64 searchStart, u64 searchEnd, u64 len, u64 alignment, u64* physAddrOut) {
    if (searchStart < 0 || searchEnd <= searchStart) {
        fprintf(stderr, "[Kernel_Memory] ERROR: Provided address range is invalid!\n");
        return ORBIS_KERNEL_ERROR_EINVAL;
    }
    if (len <= 0 || !Is64KBAligned(len)) {
        fprintf(stderr, "[Kernel_Memory] ERROR: Provided length 0x%lx is invalid!\n", len);
        return ORBIS_KERNEL_ERROR_EINVAL;
    }
    ...
    void* memory = MemoryManager_GetInstance();
    PAddr phys_addr = MemoryManager_PoolExpand(memory, searchStart, searchEnd, len, alignment);
    if (phys_addr == (PAddr)-1) {
        return ORBIS_KERNEL_ERROR_ENOMEM;
    }
    ...
    *physAddrOut = (s64)phys_addr;
    ...
    printf("[Kernel_Memory] sceKernelMemoryPoolExpand: searchStart = 0x%lx, searchEnd = 0x%lx, "
...
s32 sceKernelMemoryPoolReserve(void* addr_in, u64 len, u64 alignment, s32 flags, void** addr_out) {
    printf("[Kernel_Memory] sceKernelMemoryPoolReserve: addr_in = %p, len = 0x%lx, "
           "alignment = 0x%lx, flags = 0x%x\n", addr_in, len, alignment, flags);

    if (len == 0 || !Is2MBAligned(len)) {
        fprintf(stderr, "[Kernel_Memory] ERROR: Map size is either zero or not 2MB aligned!\n");
        return ORBIS_KERNEL_ERROR_EINVAL;
    }
    if (alignment != 0) {
    ...
    return MemoryManager_MapMemory(memory, addr_out, in_addr, len, 0, flags, 3, "anon",
                                   false, -1, map_alignment);
}
...
s32 sceKernelMemoryPoolCommit(void* addr, u64 len, s32 type, s32 prot, s32 flags) {
    ...
    if (len == 0 || !Is64KBAligned(len)) {
        fprintf(stderr, "[Kernel_Memory] ERROR: Map size is either zero or not 64KB aligned!\n");
        return ORBIS_KERNEL_ERROR_EINVAL;
    }
...
s32 sceKernelMemoryPoolDecommit(void* addr, u64 len, s32 flags) {
    ...
    printf("[Kernel_Memory] sceKernelMemoryPoolDecommit: addr = %p, len = 0x%lx, flags = 0x%x\n",
           addr, len, flags);
...
s32 sceKernelMemoryPoolBatch(const OrbisKernelMemoryPoolBatchEntry* entries, s32 count,
    ...
    for (s32 i = 0; i < count; i++, processed++) {
        ...
        case ORBIS_MEMORY_POOL_TYPE_PROTECT:
            result = sceKernelMtypeprotect(entry.type_protect_params.addr, entry.type_protect_params.len,
                                          entry.type_protect_params.type, entry.type_protect_params.prot);
            break;
        case ORBIS_MEMORY_POOL_MOVE:
            fprintf(stderr, "[Kernel_Memory] ERROR: Unimplemented memory pool opcode Move\n");
            result = ORBIS_KERNEL_ERROR_EINVAL;
            break;
        default:
            result = ORBIS_KERNEL_ERROR_EINVAL;
...
void* posix_mmap(void* addr, u64 len, s32 prot, s32 flags, s32 fd, s64 phys_addr) {
    ...
    if (flags & 0x1000) { // MAP_ANON
        // VMAType::Flexible = 2
        result = MemoryManager_MapMemory(memory, &addr_out, aligned_addr, aligned_size,
        ...
        // File mapping
        result = MemoryManager_MapFile(memory, &addr_out, aligned_addr, aligned_size,
                                      prot, flags, fd, phys_addr);
    }
...
s32 sceKernelSetPrtAperture(s32 id, VAddr address, u64 size) {
    ...
    printf("[Kernel_Memory] WARNING: PRT aperture id = %d, address = 0x%lx, size = 0x%lx "
           "is set but not used\n", id, address, size);
...
void RegisterKernelMemory(void* symbol_resolver) {
    ...
    // SymbolResolver_Register(symbol_resolver, "usHTMoFoBTM", sceKernelEnableDmemAliasing);
    // SymbolResolver_Register(symbol_resolver, "rTXw65xmLIA", sceKernelAllocateDirectMemory);
