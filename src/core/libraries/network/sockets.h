...
typedef INT(PASCAL LPFNWSASENDMSG)(
...
typedef INT(PASCAL LPFNWSARECVMSG)(
...
struct Socket {
  ...
  // Get socket options
  virtual int GetSocketOptions(int level, int optname, void *optval,
                               u32 optlen) = 0;
  ...
  virtual int SendPacket(const void *msg, u32 len, int flags,
                         const OrbisNetSockaddr *to, u32 tolen) = 0;

  // Accept an incoming connection
  virtual std::shared_ptr<Socket> Accept(OrbisNetSockaddr *addr,
                                         u32 *addrlen) = 0;
  ...
  virtual int ReceivePacket(void *buf, u32 len, int flags,
                            OrbisNetSockaddr *from, u32 *fromlen) = 0;
  ...
  virtual std::optional<netsocket> Native() = 0;
};
} // namespace Libraries::Net
