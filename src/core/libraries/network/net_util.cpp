...
const std::string& NetUtilInternal::GetIp() const {
    if (Common::Singleton<Core::Networking::NetworkingCore>::Instance()->getNetworkingConfig().mode ==
        Core::Networking::Mode::LAN_Play) {
        return Common::Singleton<Core::Networking::NetworkingCore>::Instance()
            ->getNetworkingConfig()
            .guest_ip;
    }
    return ip;
...
int NetUtilInternal::ResolveHostname(const char* hostname, Libraries::Net::OrbisNetInAddr* addr) {
    std::string resolved_hostname =
        Common::Singleton<Core::Networking::NetworkingCore>::Instance()->ResolveHostname(hostname);
    const char* final_hostname = resolved_hostname.c_str();
    ...
    auto gai_result = getaddrinfo(final_hostname, nullptr, &hints, &info);
