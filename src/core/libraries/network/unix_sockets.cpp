...
// ----  Convert Orbis sockaddr → Unix sockaddr  ----
static int ConvertOrbisNetSockaddrToUnix(const OrbisNetSockaddr *in,
                                         struct sockaddr *out, u32 *len) {
...
// ----  Unix-domain socket stubs  ----
int PS4_SYSV_ABI sys_socket_unix(int domain, int type, int protocol) {
  LOG_ERROR(LibNet, "(STUBBED) Unix socket creation domain={} type={} proto={}",
            domain, type, protocol);
  return 42; // fake fd
}

int PS4_SYSV_ABI sys_connect_unix(int sockfd, const struct sockaddr_un *addr,
                                  u32 addrlen) {
  LOG_ERROR(LibNet, "(STUBBED) Unix connect fd={}", sockfd);
  return ConvertReturnErrorCode(0);
}

int PS4_SYSV_ABI sys_bind_unix(int sockfd, const struct sockaddr_un *addr,
                               u32 addrlen) {
  LOG_ERROR(LibNet, "(STUBBED) Unix bind fd={}", sockfd);
  return ConvertReturnErrorCode(0);
}

int PS4_SYSV_ABI sys_accept_unix(int sockfd, struct sockaddr_un *addr,
                                 u32 *addrlen) {
  LOG_ERROR(LibNet, "(STUBBED) Unix accept fd={}", sockfd);
