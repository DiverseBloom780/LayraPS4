...
bool layra_pkg_parse_header(FILE *pkg_file, layra_pkg_header_t *header) {
  ...
  // digest_entries1, digest_entries2, digest_table_digest, digest_body_digest
  // are char arrays
...
bool layra_pkg_parse_entries(FILE *pkg_file, layra_pkg_header_t *header,
  ...
  uint32_t num_entries =
      header
          ->pkg_file_count; // Assuming pkg_file_count is the number of entries
  *entries =
  ...
  if (last_slash) {
    *last_slash = '\0';
    ...
    if (stat(dir_path, &st) == -1) {
      ...
      // This is a simplified version, real implementation might need recursive
      // creation
...
bool layra_pkg_open_and_mount(const char *pkg_filepath,
  ...
  for (uint32_t i = 0; i < header.pkg_file_count; ++i) {
    ...
    // Simplified filename for now, real implementation needs filename table
    // lookup
  ...
  // Mount the extracted content to the VFS - DISABLED
  ...
  // Just return true as we extracted successfully
